...
class BaseDataset(data.Dataset):
    def __init__(self, root, list_path, set_,
                 max_iters, image_size, labels_size, mean, augmentations=None):
        self.root = Path(root)
        self.set = set_
    ...
    def get_image_aug(self, file):
        return _load_aug_img(file, self.image_size, Image.BICUBIC, rgb=True, augmentations=self.augmentations)
...
def _load_aug_img(file, size, interpolation, rgb, augmentations):
    img = Image.open(file)
    if rgb:
        img = img.convert('RGB')
    img = img.resize(size, interpolation)
